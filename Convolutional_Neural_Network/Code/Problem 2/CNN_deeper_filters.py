...
class LeNet5(nn.Module):
    def __init__(model):
        super().__init__()
        model.conv1 = nn.Conv2d(1, 10, kernel_size=5, stride=1, padding=2)  # 28x28x1 -> 28x28x6
        model.pool1 = nn.AvgPool2d(kernel_size=2, stride=2)  # 28x28x6 -> 14x14x6
        model.conv2 = nn.Conv2d(10, 25, kernel_size=5, stride=1, padding=0)  # 14x14x6 -> 10x10x16
        model.pool2 = nn.AvgPool2d(kernel_size=2, stride=2)  # 10x10x16 -> 5x5x16
    
        model.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(25 * 5 * 5, 120),  # 16*5*5 as per the image
            nn.ReLU(),
            nn.Linear(120, 84),  
            nn.ReLU(),
            nn.Linear(84, 10),  
